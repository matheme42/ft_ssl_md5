...
/*   Updated: 2022/03/15 20:49:43 by matheme          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void whirlpool_hash(const char *message, size_t len, uint8_t hash[restrict static STATE_LEN]) {
	ft_bzero(hash, STATE_LEN * sizeof(uint8_t));
	
	#define LENGTH_SIZE 32  // In bytes
	
	size_t off;
	for (off = 0; len - off >= BLOCK_LEN; off += BLOCK_LEN)
		whirlpool_compress((const uint8_t *)&message[off], hash);
	
	uint8_t block[BLOCK_LEN] = {0};
	size_t rem = len - off;
	if (rem > 0)
		ft_memcpy(block, &message[off], rem);
...
char *whirlpool_algo(int option, int file_len, char *file)
{
	...
	hash = NULL;
	uint8_t hash_int[STATE_LEN];
	ft_bzero(hash_int, STATE_LEN);
	int i = 0;
	whirlpool_hash(file, file_len, hash_int);
	hash = ft_utoa_base_champ(hash_int[0], 16, 2);
	while (++i < STATE_LEN)
		ft_strjoin_free(&hash, ft_utoa_base_champ(hash_int[i], 16, 2));
    return (hash);
}
